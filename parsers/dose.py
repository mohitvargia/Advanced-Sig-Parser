...
class DoseOnlyParser(DoseParser):
    def normalize_pattern(self):
        ...
        pattern = re.compile(r'(?<!time\\s)\\b(?:' + r'|'.join(method_patterns) + r')?\\s?(?P<dose>' + RE_RANGE + r')(?!:)(?!(?:\\s)?(?:\\d|\\.|\\/))(?!(?:\\s)?(?:times|x|time\\b))(?!\\s?(?:' + r'|'.join(strength_unit_patterns) + r'))(?!\s?(?:days?|weeks?|months?|hours?|hrs?|minutes?|mins?|years?|yrs?)\\b)', flags = re.I)
        return pattern
    def normalize_match(self, match):
        dose_range = split_range(match.group('dose'))
        dose, dose_max = dose_range
