...
class FrequencyEveryDay(FrequencyParser):
	pattern = r'(?:every|each|q|per|\ba)\s?(?P<period>other\b|o)?\s?(?:day (?:in the|at)\s?)?(?P<period_unit>hour|day|week|wk\b|month|morning|afternoon|evening at bedtime|bedtime|evening|night|hs|pm|am|d\b)'
	def normalize_match(self, match):
		frequency = 1
		...
		frequency_text_start, frequency_text_end = match.span()
		frequency_text = match.group(0)
		raw_unit = match.group('period_unit').lower()
		time_keywords = ['morning', 'afternoon', 'evening', 'night', 'bedtime', 'hs', 'am', 'pm']
		
		# If it's a specific time of day, preserve it in readable text
		if any(k in raw_unit for k in time_keywords):
			# Clean up raw unit (e.g. 'in the morning' -> 'morning') matches are usually just the keyword part from regex group
			# But regex group might be just 'morning' based on pattern.
			# Convert abbreviations
			if raw_unit in ['hs', 'bedtime', 'evening at bedtime']:
				readable_unit = 'at bedtime'
			elif raw_unit in ['am', 'morn']:
				readable_unit = 'in the morning'
			elif raw_unit in ['pm', 'eve']:
				readable_unit = 'in the evening'
			else:
				readable_unit = raw_unit
				if 'morning' in raw_unit: readable_unit = 'in the morning'
...
class FrequencyInTheX(FrequencyParser):
	...
	def normalize_match(self, match):
		...
		frequency_text_start, frequency_text_end = match.span()
		frequency_text = match.group(0)
		frequency_readable = frequency_text # Use the specific text (e.g. 'in the morning') instead of generic 'daily'
		return self.generate_match({'frequency': frequency, 'period': period, 'period_unit': period_unit, 'frequency_text_start': frequency_text_start, 'frequency_text_end': frequency_text_end, 'frequency_text': frequency_text, 'frequency_readable': frequency_readable})
